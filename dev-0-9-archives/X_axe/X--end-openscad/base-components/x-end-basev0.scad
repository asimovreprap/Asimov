        ...
        translate([z_linear_bearing_outside_diameter + 2*2 - junction_assurance_length, 
            ...
            difference(){
                ...
                translate([z_shaft_to_leadscrew_distance - (z_linear_bearing_outside_diameter + 2*2)/2,
                    ...
                    // Holes for fixation screws for leadscrew nut
                    for(rotation = [0:3]){
                        rotate(a=[0,0,45+(90*rotation)]){
                            translate([9.5,0,0]){
                                cylinder(h=100, r=(z_leadscrew_nut_screw_diameter/2)+holes_extra_radius_compensation, center = true, $fn=10);
                            }
                        }
                    }

                    // Holes for fixation screws for leadscrew nut ( alternative )
                    for(rotation = [0:3]){
